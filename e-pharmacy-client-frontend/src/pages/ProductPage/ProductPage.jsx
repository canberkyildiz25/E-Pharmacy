...
import { fetchMedicineDetail } from '../../store/slices/medicineSlice'
...
function ProductPage() {
  ...
  const handleAddToCart = async () => {
    ...
    if (updateCart.fulfilled.match(result)) toast.success(`${quantity}x ${detail.name} added to cart!`)
    else toast.error('Failed to add')
  }

  if (loading) return <div className={styles.center}><p>Loading...</p></div>
  if (!detail) return <div className={styles.center}><p>Product not found.</p></div>
  ...
  return (
    <div className={styles.page}>
      <div className={styles.container}>
        <button className={styles.backBtn} onClick={() => navigate(-1)}>← Back to medicines</button>
          ...
          <div className={styles.info}>
            ...
            {avgRating && (
              <div className={styles.ratingRow}>
                ...
                <span className={styles.ratingText}>{avgRating} ({reviews.length} reviews)</span>
              </div>
            )}
        ...
        <div className={styles.tabs}>
          <button className={`${styles.tab} ${tab === 'description' ? styles.activeTab : ''}`} onClick={() => setTab('description')}>Description</button>
          <button className={`${styles.tab} ${tab === 'reviews' ? styles.activeTab : ''}`} onClick={() => setTab('reviews')}>
            Reviews {reviews.length > 0 && <span className={styles.reviewCount}>{reviews.length}</span>}
          </button>
        </div>

        {tab === 'description' ? (
          <div className={styles.tabContent}>
            {detail.description ? <p className={styles.desc}>{detail.description}</p> : <p className={styles.empty}>No description available.</p>}
            {detail.warnings && (
              <div className={styles.warnings}>
                <h3 className={styles.warningTitle}>⚠️ Warnings</h3>
